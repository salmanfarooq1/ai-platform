...
**Trade-off:** SSE connections are long-lived. Each connected MCP client holds one open HTTP connection for the session duration. On Koyeb's free tier with limited concurrent connections, practical concurrency is limited. Acceptable for development and single-user demo deployments. For multi-user production, Streamable HTTP transport would replace SSE.
